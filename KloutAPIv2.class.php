...
/******************************************************/
        ...
        public function KloutIDLookupByName($network, $screenname){
                ...
                if (is_object($ResultString)) {
                        if (property_exists($ResultString, 'id')) {
                                return $ResultString->id;
                        } else {
                                throw new Exception('Server error on Klout service:' . $CurlResult);
                        }
                } else {
                        throw new Exception('Server error on Klout service. Response data is not object' . $CurlResult);
                }
        }
        /** 
         * KloutIDLookupByID
...
/******************************************************/
        ...
        public function KloutScore($id){
                ...
                $ResultString = json_decode($CurlResult);
                if (is_object($ResultString)) {
                        if (property_exists($ResultString, 'score')) {
                                return $ResultString->score;
                        } else {
                                throw new Exception('Server error on Klout service:' . $CurlResult);
                        }
                } else {
        ...
        public function KloutScoreChanges($id, $period){
                ...
                if (is_object($ResultString)) {
                        if ($period == "day") {
                                if (property_exists($ResultString, 'scoreDelta') && property_exists($ResultString->scoreDelta, 'dayChange')) {
                                        $KloutScoreChanges = $ResultString->scoreDelta->dayChange;
                                } else {
                                        throw new Exception('Server error on Klout service:' . $CurlResult);
                                }
                        } elseif ($period == "week") {
                                if (property_exists($ResultString, 'scoreDelta') && property_exists($ResultString->scoreDelta, 'weekChange')) {
                                        $KloutScoreChanges = $ResultString->scoreDelta->weekChange;
                                } else {
                                        throw new Exception('Server error on Klout service:' . $CurlResult);
                                }
                        } else {
                                if (property_exists($ResultString, 'scoreDelta') && property_exists($ResultString->scoreDelta, 'monthChange')) {
